...
Y_n=interpolate(temp,ME)
Z_n=interpolate(temp,ME)

def f(u):
	return u**3-u*beta**2

g=Expression('x[0]*x[1]*x[0]-beta',beta=beta,degree=2)
aY=Y*v*dx+0.5*con*dt*dot(grad(Y),grad(v))*dx
LY=X_n*v*dx
...
aX=X*w*dx+0.5*con*dt*dot(grad(X),grad(w))*dx
LX=Z_n*w*dx+dW*w*dx
#+0*w*dx
